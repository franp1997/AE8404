...
```
5. Implementar una función `densidad.est.parzen()` que dados un conjunto de datos, una ventana `h` y un punto `x` y devuelva $fh(x)$, el valor de la estimación de la densidad $f$ en el punto $x$, utilizando el núcleo uniforme (también llamado rectangular).  
...
>_Si se efectúa un cambio de variable $t = (x-X_i)/h$ la función indicadora queda definida como: $\pmb{I}[X_i \in (x(t)-h,x(t)+h) ] = \pmb{I}[-1<t<+1]$_. Si definimos una función _núcleo_ como $K(t)=1/2 \  \pmb{I}[-1<t<+1]$, el estimador de densidad puede escribirse en términos de dicha función como:_ 

$$\hat f_h(x) \approx \frac{1}{h n} \sum_{i=1}^{n} K \left( \frac{x-X_i}{h} \right)$$

> _A partir de esta definición, la siguiente rutina implementa la función del núcleo uniforme Ku(u)y el estimador de densidad `densidad.est.parzen`_
...
ggplot(data=DATA[kernel=="rectangular"],aes(x=x,y=fh,linetype=kernel,color=bw))+geom_line()+ theme_bw()
ggplot(data=DATA[kernel=="gaussian"],aes(x=x,y=fh,linetype=kernel,color=bw))+geom_line()+ theme_bw()
ggplot(data=DATA[kernel=="epanechnikov"],aes(x=x,y=fh,linetype=kernel,color=bw))+geom_line()+ theme_bw()
```

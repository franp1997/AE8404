...
```{r echo=TRUE}
Coeff <- function(data=NULL,y,x=NULL){
...
_Para el modelo cúbico incompleto el estimador adoptado fue $\hat{P} = \hat{\beta_0} + \hat{\beta_1} D^3$_ 

```{r echo=TRUE}
DATA <- DT[,.(P=P,x1=D^3)]
M3 <- Coeff(data=DATA,y="P")
P3  <- DT[,.(x=D,y=M3$b0+M3$b1*D^3)] #Predictor
```
```{r}
ggplot() +   
  geom_point(data=DT[,.(P,D)],aes(x=D, y=P),color= "red") +  
